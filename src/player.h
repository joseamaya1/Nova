...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void sendDamageMessage(const std::string& message, const Position& pos,
								TextColor_t Color = TEXTCOLOR_NONE) {
			if (client) {
				client->sendDamageMessage(message, pos, Color);
			}
		}
		void sendExperienceMessage(const std::string& message, const Position& pos, TextColor_t color) {
			if (client) {
				client->sendExperienceMessage(message, pos, color);
			}
		}
		//container
