...
#define dispatchDisconnectClient(err) g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::disconnectClient, this, err)))
	...
	if (accountName == 0) {
		dispatchDisconnectClient("You must enter your account number.");
		return;
	}
	...
	if (accountId == 0) {
		dispatchDisconnectClient("Account number or password is not correct.");
		return;
	}
...
void ProtocolGame::sendContainer(uint8_t cid, const Container* container, bool hasParent, uint16_t firstIndex)
{
	...
	for (ItemDeque::const_iterator it = itemList.begin() + firstIndex, end = itemList.end(); i < 255 && it != end; ++it, ++i) {
		msg.addItem(*it);
	}
...
void ProtocolGame::sendDamageMessage(const std::string& message, const Position& pos,
                                     TextColor_t Color/* = TEXTCOLOR_NONE*/)
{
	NetworkMessage msg;
	AddAnimatedText(msg, pos, Color, message);
	writeToOutputBuffer(msg);
}

void ProtocolGame::sendExperienceMessage(const std::string& message, const Position& pos, TextColor_t color)
{
	NetworkMessage msg;
	AddAnimatedText(msg, pos, color, message);
	writeToOutputBuffer(msg);
